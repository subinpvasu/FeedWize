...
//exit();
  ...
  public function ModifyAccount(AdWordsServices $adWordsServices, AdWordsSession $session,$status = "TOGGLE", $leave_some=0) {
...
//                    $this->UpdateCampaign($adWordsServices,$session, $campaign->getId(), 0);
		...
		case "1" : 
//                    UpdateCampaign($user, $campaign->id, "ENABLED");	 
                    $this->UpdateCampaign($adWordsServices,$session, $campaign->getId(), 1);
                    break;
		case "2" :
		case "0" : if ($leave_some)
							{
							if (!stristr($campaign->getName(), "apples"))
							if (!stristr($campaign->getName(), "oranges"))
							
//							UpdateCampaign($user, $campaign->id, "PAUSED");	
                                                        $this->UpdateCampaign($adWordsServices,$session, $campaign->getId(), 0);
                                                        break;
							}
						else
//						UpdateCampaign($user, $campaign->id, "PAUSED");	
                                                $this->UpdateCampaign($adWordsServices,$session, $campaign->getId(), 0);
                                                break;
		//case "TOGGLE" : if ($campaign->status == "PAUSED")
		//				UpdateCampaign($user, $campaign->id, "ACTIVE");	
	//					else if ($campaign->status == "ACTIVE")
	//					UpdateCampaign($user, $campaign->id, "PAUSED");	
		//case "TOGGLE" : if ($campaign->getStatus == "PAUSED")
		//				 $this->UpdateCampaign($adWordsServices,$session, $campaign->getId(), 1);
	//					else if ($campaign->getStatus == "ACTIVE")
	//					$this->UpdateCampaign($adWordsServices,$session, $campaign->getId(), 0);
		}
